---
company:
 - innblockchain
department:
 - marketing
priority: high
rev: 1
last_revised: 2026-06-13
parent_extraction_source: "[[InnBlockchain - Phase 1 Execution Playbook]] § WS7 § Per-article AEO/GEO checklist + § Per-article sales-motion tag"
scope: cross-phase
applies_to:
 - phase-1
 - phase-2
 - phase-3
tags:
 - framework
 - pre-publish
 - aeo
 - sales-motion
 - cw
 - cross-phase
---

# Per-Article Pre-Publish Checklists

> [!INFO] Two checklists CW runs before submitting an article for review · cross-phase framework
> Both fire at Phase 2 of the [[InnBlockchain - Per-Article Production Checklist]] (steps 9 + 10). Pull these out as a separate doc because the 9 + 4 items are universal across all phases — only the prompt-block source (per-phase per-article prompts) updates per phase.

## Checklist 1 — AEO/GEO drafting gate (9 items · every article)

- [ ] **(CW)** Pull per-article prompt block from [[InnBlockchain - Content AEO GEO Prompts]] § (phase-specific) Week-by-Week Article Prompts
- [ ] **(CW)** H2/H3 headings ≈ prompts verbatim (Principle 2)
- [ ] **(CW)** 150-word answer rule per H2 (Principle 1)
- [ ] **(CW)** FAQPage JSON-LD (Principle 3)
- [ ] **(CW)** Cite specifics: dates, version numbers, named competitors (Principles 4 + 5)
- [ ] **(CW)** Date-stamp regulation (Principle 6)
- [ ] **(CW)** Markdown tables (Principle 7)
- [ ] **(CW)** Persona-aligned vocabulary (Principle 8)
- [ ] **(CW)** No marketing voice (Principle 9)

## Checklist 2 — Sales-motion tag (4 items · mandatory before submitting for review)

Per [[InnBlockchain - Content Strategy]] § Sales Motion Tagging:

- [ ] **(CW · Vignesh confirms)** Track tag: `[CN]` / `[FT]` / `[Both]`
- [ ] **(CW · Vignesh confirms)** Sales motion: `[Audit]` / `[Dev]` / `[White-Label]` / `[Audit + Dev]`
- [ ] **(CW · Vignesh confirms)** Destination LP (track-specific even when topic shared)
- [ ] **(CW · Vignesh confirms)** CTA matching the sales motion (no motion mixing in one article's CTA — `[Both]` uses split CTA footer)

## Why this is a separate doc

13 universal items (9 AEO + 4 sales-motion). Phase-specific overlays:
- AEO Principle 8 persona-aligned vocabulary updates when new ICP personas activate
- Sales motion `[White-Label]` motion phases in/out based on which productized offerings are live per phase
- Track tags expand if new ICP tracks activate (e.g., Phase 2 adds DeFi-protocol track)

Otherwise the checklist structure is stable.
