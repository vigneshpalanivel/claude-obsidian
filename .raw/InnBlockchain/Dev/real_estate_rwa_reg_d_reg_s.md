...
1. **U.S. persons may buy ONLY if they are accredited AND verified, and ONLY in the Reg D (506(c)) tranche.** A U.S. person who is not accredited, or is accredited but not verified under Rule 506(c)(2)(ii), cannot subscribe at all. (This governs **primary issuance** only — *subscription*. Secondary resale of a `REG_D` token after its Rule 144 holding period clears is governed by Rule 144, not by accreditation, and may reach any eligible U.S. person; see Section D and the Section B gate.) Offshore non-U.S. persons subscribe in the **Reg S** tranche. **The Reg S tranche still blocks EVERY U.S. person** (even a verified-accredited one) for the life of that partition's distribution compliance period — a U.S. accredited buyer belongs in a `REG_D` partition, never a `REG_S` partition. Verified U.S. accredited investors and offshore investors are kept in separate partitions so the two exemptions do not integrate (Rule 152(b)(2)).
...
- [ ] **§12(g) holder-count monitor (Exchange Act)** — accumulated holders across **BOTH** tranches can trip mandatory Exchange Act registration at **2,000 holders of record, or 500 non-accredited holders, plus >$10M in assets** (checklist §12.3). The 9.99% cap limits *concentration*, not *count* — nothing on-chain caps holder numbers. **The post-144 resale relaxation (Section B/D) raises this exposure**: washed `REG_D` tokens can now reach non-accredited U.S. persons, and Reg S non-accredited offshore holders already feed the 500-prong. Assign the compliance team to track the class-wide holder count (reconstructed off-chain from the transfer event stream) against these thresholds and alert as they approach. Cross-ref: [reg-d-checklist §12.3](../sales-marketing/Service/Content/US%20Compliance/reg-d-checklist.md)
...
- [ ] `FirstSaleRecorded(tranche, timestamp)` — emitted on the first mint in each tranche. It is a **backstop** for the **15-day Form D** clock (Rule 503), **not** its legal trigger: the clock runs from the first *irrevocable subscription commitment*, which can predate the mint (fiat settlement, §I.2). The off-chain compliance team files on EDGAR within 15 calendar days **of subscription acceptance**, using this event only as a latest-possible bound. Also feeds the state notice-filing workflow (Section 18 covered-security notices)
...
- [ ] **Scope — primary issuance only.** This verified-accredited gate is the *mint* (primary-sale) rule. It does NOT govern secondary resale: once a holder's lot clears its Rule 144 holding period, a non-affiliate resale drops the accreditation requirement and may reach any eligible U.S. person (Section B reject-list / Section D). Do not carry the `US_ACCREDITED_VERIFIED` mint gate into the secondary-transfer path
