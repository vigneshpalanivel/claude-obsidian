...
1. **U.S. persons may buy ONLY if they are accredited AND verified, and ONLY in the Reg D (506(c)) tranche.** A U.S. person who is not accredited, or is accredited but not verified under Rule 506(c)(2)(ii), cannot subscribe at all. (This governs **primary issuance** only — _subscription_. Secondary resale of a `REG_D` token after its Rule 144 holding period clears is governed by Rule 144, not by accreditation, and may reach any eligible U.S. person; see Section D and the Section B gate.) Offshore non-U.S. persons subscribe in the **Reg S** tranche. **The Reg S tranche still blocks EVERY U.S. person** (even a verified-accredited one) for the life of that partition's distribution compliance period — a U.S. accredited buyer belongs in a `REG_D` partition, never a `REG_S` partition. Verified U.S. accredited investors and offshore investors are kept in separate partitions so the two exemptions do not integrate (Rule 152(b)(2)).
...
| U.S.-person gate               | Primary mint / pre-144 transfer: recipient must be `US_ACCREDITED_VERIFIED`. Post-144 non-affiliate resale: any KYC'd / OFAC-clear / under-cap U.S. person (washed — accreditation not required) | ALL U.S. persons blocked during the compliance period |
...
- [ ] **Pre-existing bad-actor disclosure (Rule 506(e))** — a triggering event that occurred **before 23 September 2013** does NOT disqualify the offering, but MUST be **disclosed in writing to purchasers a reasonable time before sale**. The bad-actor questionnaires (above) surface these; any predating the cutoff go into the PPM / a written disclosure supplement whose hash is bound as an ERC-1643 document (Section E). Off-chain drafting duty; the platform only anchors the document hash. Cross-ref: [reg-d-checklist §8.3](../sales-marketing/Service/Content/US%20Compliance/reg-d-checklist.md)
- [ ] **Form D (Rule 503)** — file on EDGAR within **15 calendar days after the first sale** in the 506(c) tranche; amend annually / on material change. **"First sale" is the date the first investor becomes _irrevocably contractually committed_ (subscription acceptance) — which may PRECEDE the on-chain mint**, especially on the fiat path where `issueByPartition` mints only after off-chain settlement (§I.2). The compliance team therefore runs the 15-day clock from **subscription acceptance**, and treats the `FirstSaleRecorded(tranche, timestamp)` event as a **backstop / latest-possible bound**, not the trigger — relying on the mint event alone risks a late filing (Rule 507 exposure). Cross-ref: [reg-d-checklist §9](../sales-marketing/Service/Content/US%20Compliance/reg-d-checklist.md)
...
- [ ] **§12(g) holder-count monitor (Exchange Act)** — accumulated holders across **BOTH** tranches can trip mandatory Exchange Act registration at **2,000 holders of record, or 500 non-accredited holders, plus >$10M in assets** (checklist §12.3). The 9.99% cap limits _concentration_, not _count_ — nothing on-chain caps holder numbers. **The post-144 resale relaxation (Section B/D) raises this exposure**: washed `REG_D` tokens can now reach non-accredited U.S. persons, and Reg S non-accredited offshore holders already feed the 500-prong. Assign the compliance team to track the class-wide holder count (reconstructed off-chain from the transfer event stream) against these thresholds and alert as they approach. Cross-ref: [reg-d-checklist §12.3](../sales-marketing/Service/Content/US%20Compliance/reg-d-checklist.md)

### What is identical across both tranches

- [ ] **OFAC** screening (Section A.1), **KYC**, the **affiliate-free architecture** (no affiliate holds tokens in either tranche), the **9.99% class-wide concentration cap** (aggregated across BOTH tranches — it is one class of equity), and **Rule 144** restricted-securities resale all apply uniformly. The tranche type changes only the _primary-issuance eligibility_ and whether a Reg S distribution compliance period runs
...
- **Rule 506(c) conditions** (`REG_D` partitions) — the recipient must be `US_ACCREDITED_VERIFIED` for a **primary mint** or for any **secondary transfer of a lot still inside its Rule 144 holding period** (the security is still restricted and in motion; tacking applies). **Once a lot's Rule 144 holding period has cleared** (12 months, non-affiliate — guaranteed by the affiliate-free architecture), the non-affiliate resale "washes" the restriction (Section D) and the recipient may be **any KYC'd, OFAC-clear, under-cap U.S. person — accredited or not** (Rule 144 does not gate the resale buyer's accreditation). There is no Reg S distribution compliance period on a `REG_D` partition
...
- [ ] **[`REG_D` partition]** On a **primary mint** OR a **secondary transfer of a lot whose Rule 144 holding period has NOT yet cleared** (`block.timestamp < holdingEnd[from][partition]`), the recipient is a **U.S. person who is NOT `US_ACCREDITED_VERIFIED`** (accredited AND verified under Rule 506(c)(2)(ii)) → reason: `"REG_D_NOT_VERIFIED_ACCREDITED"`. A single non-verified U.S. purchaser in the primary offering breaks 506(c) for the tranche, and a still-restricted token must not reach an unverified U.S. person. **After the lot's Rule 144 holding period clears** (`block.timestamp >= holdingEnd[from][partition]`; the seller is always a non-affiliate under the affiliate-free architecture), this accreditation gate **drops** — the resale washes the token clean (Section D) and the recipient may be **any U.S. person**, provided they still pass the tranche-independent gates below (in registry / KYC fresh / not `BLOCKED` / OFAC-clear / within the 9.99% cap). Accreditation is a primary-issuance condition, never a Rule 144 resale condition
...
- On a **`REG_D` partition**, Rule 144 is the **only** U.S.-resale gate — there is no Reg S compliance period. Once the holding period clears, the non-affiliate resale recipient may be **any** eligible U.S. person (accredited or not; the token washes clean under Rule 144(b)(1)(ii)) — the `canTransfer` `REG_D` branch drops its accreditation requirement at `holdingEnd` (Section B). The tranche-independent gates (registry / KYC / OFAC / `BLOCKED` / 9.99% cap) still apply to every hop.
...
- [ ] `FirstSaleRecorded(tranche, timestamp)` — emitted on the first mint in each tranche. It is a **backstop** for the **15-day Form D** clock (Rule 503), **not** its legal trigger: the clock runs from the first _irrevocable subscription commitment_, which can predate the mint (fiat settlement, §I.2). The off-chain compliance team files on EDGAR within 15 calendar days **of subscription acceptance**, using this event only as a latest-possible bound. Also feeds the state notice-filing workflow (Section 18 covered-security notices)
...
- [ ] **Recipient is `US_ACCREDITED_VERIFIED`** — the identity record carries both the `accredited` flag (a Rule 501(a) category) AND a completed **verification record** (`verified = true`, not expired). Self-certification does NOT satisfy this; `mint` reverts with `REG_D_NOT_VERIFIED_ACCREDITED` otherwise (Rule 506(c)(2)(ii))
  - **Platform defense-in-depth (implementation note): explicit `US_PERSON` gate + `REG_D_NON_US_PERSON` reason code.** `US_ACCREDITED_VERIFIED` is defined as a _U.S. person_ who is accredited and verified, so in principle a non-U.S. recipient already fails this gate. But because accreditation is a wealth/status test that is **nationality-independent** (a non-U.S. person can be accredited), the platform does not rely on the classification bundling U.S.-person status implicitly. On the mint / pre-holding-period path it **additionally** checks the `US_PERSON` claim and reverts a non-U.S. recipient with a distinct `REG_D_NON_US_PERSON` code — so a mis-recorded accreditation on a non-U.S. wallet still cannot enter `REG_D`, and the reason is truthful (nationality, not lack of verification). This makes the "restricted class = `US_PERSON`" model (which the Rule 144 holding gate depends on) hold for every `REG_D` holder. Non-U.S. accredited investors are routed to `REG_S`. See the platform architecture decision record §2.1 (Design A). This is a stricter platform enforcement, not a Regulation D requirement — Reg D itself does not bar non-U.S. accredited purchasers
...
- [ ] **Signed subscription agreement hash** present, containing the purchaser's accredited-investor representations, the **Rule 502(d)(i) own-account inquiry** (a representation that the purchaser acquires for their **own account** and not with a view to distribution — not as a nominee or conduit for an undisclosed other; the reasonable-inquiry limb of Rule 502(d)), and the **Rule 502(d) restricted-securities acknowledgement + resale covenant** (resell only under Rule 144, registration, or another exemption)
...
- [ ] **Scope — primary issuance only.** This verified-accredited gate is the _mint_ (primary-sale) rule. It does NOT govern secondary resale: once a holder's lot clears its Rule 144 holding period, a non-affiliate resale drops the accreditation requirement and may reach any eligible U.S. person (Section B reject-list / Section D). Do not carry the `US_ACCREDITED_VERIFIED` mint gate into the secondary-transfer path
...
- [ ] **The target partition is still open** — `mint` is allowed iff `block.timestamp < partition.closeDate` (an unset `closeDate == max` → open). It reverts once the `closeDate` has **arrived** (`block.timestamp >= closeDate`) — NOT merely because a future `closeDate` has been set (Section C, invariant 2). No sales after the close. New subscriptions after a close go into a fresh partition
  - **`REG_D` partitions have no `closeDate` cutoff** (`closeDate == max`, and `setCloseDate` is prohibited for `REG_D` — a finite value would falsely imply a Reg S compliance period). The `closeDate` mint gate therefore never fires for `REG_D`; primary issuance is stopped by the partition's **manual `mintingOpen` gate** — an owner/multi-sig `setMintingOpen(partition, false)` — which is reversible (stop and later resume subscriptions). This gate is available to both tranches but plays different roles: on `REG_D` it is the _only_ on-demand mint-stop; on `REG_S` it is a **temporary pause only** — **closing** a Reg S offering is still `setCloseDate` (which stops mints _and_ starts the 12-month compliance clock, Section C). Pausing a `REG_S` partition with `setMintingOpen(false)` while leaving `closeDate == max` halts sales but never starts the compliance period → the indefinite-restriction trap (Section C). (A finite `maxSupply` cap is an optional, independent stop.)
...
| **Reg D Form D filing** — EDGAR within 15 days of first `REG_D` sale; annual + material-change amendments (Rule 503)                                                                                     | Compliance officer via EDGAR (clock runs from first irrevocable subscription commitment; `FirstSaleRecorded` is a backstop, not the trigger) |
...
| §13(d) / §16 / §12(g) Exchange Act reporting (§8.2) — **incl. active holder-count monitoring vs the 2,000 / 500-non-accredited + $10M §12(g) thresholds** (heightened by the post-144 resale relaxation) | Issuer's separate SEC filings + compliance-team holder-count monitor (off-chain, from the transfer event stream)                             |
...
- [ ] **Test coverage of tranche dispatch in `canTransfer`:** a U.S. person (even accredited/verified) is BLOCKED into a `REG_S` partition (`REG_S_US_PERSON_RESTRICTED`); on a **primary mint or a pre-holding-period `REG_D` transfer**, a non-`US_ACCREDITED_VERIFIED` recipient is BLOCKED (`REG_D_NOT_VERIFIED_ACCREDITED`); a `US_ACCREDITED_VERIFIED` recipient is ALLOWED into a `REG_D` partition; a `REG_D` partition never consults `closeDate` (stays `type(uint256).max`) and its only U.S.-resale gate is Rule 144
- [ ] **Test coverage of the `REG_D` post-144 resale relaxation:** a secondary `REG_D` transfer of a lot whose Rule 144 holding period has **cleared** (`block.timestamp >= holdingEnd[from][partition]`), from a non-affiliate sender to a **non-accredited but KYC'd / OFAC-clear / under-cap U.S. person**, is **ALLOWED** (the wash); the _same_ transfer while the lot is **still inside** its holding period is **BLOCKED** (`REG_D_NOT_VERIFIED_ACCREDITED`); and the tranche-independent gates (unregistered recipient, stale KYC, `BLOCKED` sender, OFAC flag, 9.99% cap) still reject a post-144 transfer when any of them fails
...
> 💡 **The contrast that makes the dual-tranche point:** Investor D's `REG_D` tokens became U.S.-resaleable at **T+12mo** (Rule 144 only). The _same_ investor's Reg S-side tokens would not clear until **T+21mo** (`closeDate + 12mo`, because the Reg S compliance period is the binding gate on that partition). Same investor, same issuer, same day of purchase — **9 months' difference, entirely because of which tranche the tokens live in.** That is why the partitions are kept separate and why `canTransfer` dispatches on tranche.
