...
It was prospectus-blocking, and that is why it is settled now rather than at filing: the token standard is a disclosure item under Prospectus Arts 6/16(1), so changing it after approval is a material change carrying an Art 23 supplement and an investor withdrawal window. **The same logic applies to the wording of the claim** — "ERC-3643 compliant" unqualified is an overstatement, because `IIdentityRegistry` carries declared deviations, `IIdentityRegistryStorage` and `IIdentity` are declared not-implemented, and there is **no ERC-173 owner surface** (D-A2). §2 of the conformance register gives the sentence you may use. ⚠️ **Do not quote this file's grade from memory — it was wrong for a full revision.** Until 2026-09-11 the register graded four of the standard's seven interfaces and reported clean, because the conformance test diffed the contracts against **our own transcription** of the EIP rather than against the EIP. See §6 of the register for the method that replaces it.
...
10. **`Classifier.fallbackValue` polarity is unenforceable on-chain.** The classification axes that
    ...
    weakens.** The only guards available are `DEPLOYMENT-DEFAULTS.md` and a configuration review.
    ⚠️ Related, and genuinely closed rather than deferred: moving the MiFID tier out of the
    `Person` struct and into the `_class` mapping took it **out of `delete _persons[personId]`**,
