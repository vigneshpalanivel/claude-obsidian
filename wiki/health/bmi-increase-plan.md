---
tags: [health, nutrition, fitness, bmi]
created: 2026-07-06
status: active
---

# BMI Increase Plan — Vignesh

> 🎯 **This plan is Milestone 1–2 of [[healthspan-80]]** — the master plan for staying active and independent to age 80. Read that file for why this one matters beyond the number on the scale.
>
> **The window is closing:** muscle mass peaks at age 30–35. At 34 and BMI 17.4 you are inside your peak-building window while underweight. Muscle built now is banked for the next 45 years; muscle not built now is permanently forgone (anabolic resistance makes building at 60 several times harder). **This is the most time-sensitive item in the entire health folder.**
>
> ⚠️ **Target correction:** 70 kg (Phase 1) is the **floor**, not the goal. **75 kg (Phase 2) is the actual destination** and is not optional — see [[healthspan-80]] Pillar 1.

## Current Status

| Metric | Current | Target |
|--------|---------|--------|
| Height | 184cm | — |
| Weight | **59.75kg** (Aug 3, 2026) | Phase 1: **70kg** · Phase 2: **73–75kg** |
| BMI | **~17.6** | Phase 1: ~20.7 · Phase 2: ~21.5–22.2 |
| Timeline | — | Phase 1: ~11 months · Phase 2: +5–8 months |
| Monthly gain target | — | 0.8–1kg/month |

**Root cause of underweight:** Originally low protein intake (low urea 14.8, ref 17–51). **Update (Jul 2026): protein is now largely fixed — see meal-log reconciliation below. The remaining blockers are consistency, fiber, and dinner quality, not protein.**

---

## Daily Targets

| Metric | Target |
|--------|--------|
| Total calories | 2300–2500 kcal/day |
| Protein | 100–120g/day |
| **Fiber** | **25–30g/day (from calorie-dense sources only)** |
| Carbs | 280–320g/day |
| Healthy fats | 70–80g/day |

---

## Progress vs Plan — from [[meal-log]] (Jul 6–11, 2026)

What the log actually shows, vs what this plan assumed when written:

| Assumption in plan | Reality from meal-log | Action |
|--------------------|----------------------|--------|
| Intake ~1,800 kcal, protein 40–50g | Protein **hit hard on 4/6 days** (129g, 176g, 122g); calories 2,400–2,610 on good days | Protein premise outdated — **protein is no longer the bottleneck** |
| — | Jul 6–7 low (~1,528/46g, ~1,633/70g) were **pre-diet / ramp-up baseline** — structured weight-gain diet began *this week*; Jul 8–10 (2,400–2,610 kcal) are the diet working | Judge the plan from Jul 8 on, not the ramp-up days |
| PB scheduled as "Week 2 addition" | User **pulled peanut butter into week 1** — already daily in breakfast base | Good — accelerate calorie-dense adds when tolerated; update progression below |
| Fiber not tracked | **Under target every single day (~11–22g)**; all fiber rides on the Buddha bowl; constipation Jul 11 | Add daily fiber independent of the bowl |
| Dinner = grilled/boiled chicken | User **can't take chicken/fish at dinner** (eggs/paneer/dal only); actual dinners were prawn, tandoori, ghee roast | **Correct dinner protein below** |
| Avoid prawns (purine) | Diet is **prawn-heavy** (Jul 9: 5 prawn bhorta) | Fine at uric acid 5.3, but don't stack daily — vary protein |

> **Reframed problem:** Diet started this week; Jul 8–10 show it works (2,400–2,610 kcal, protein hit). Now that the ceiling is proven, the thing to protect against going forward is the **light day** — a nuts-only breakfast + skipped/banana dinner would drop you ~900 kcal and ~50g protein in one go (the Jul 6–7 ramp-up pattern). Weight gain is lost on light days, not won on good ones. Consistency > ceiling.

---

## Meal Plan

> **Daily Calorie Split: Breakfast 800–1,000 · Lunch 800–1,000 · Dinner ≤800 (patch to ~2,400).** Front-load the day — heaviest early when you can digest and burn it; dinner is the adjustable patch, kept light (~400) when B+L hit the top. **Hold the 800 floor on breakfast and lunch** — that's the one rule the whole system depends on. Full logic in [[meal-log]] "Daily Calorie Split."
> Heavy/fried food at dinner → overnight gas → morning headache → poor sleep → slower gain. An 800 dinner is a *medium* patch, not the routine — light dinners win.

### Breakfast (7:00–7:30 AM) — target 800–1,000
**Current base** (see [[meal-log]]):
- 2 tbsp peanut butter + 2 egg omelette (butter)
- Nuts: 5 almonds + 10 pumpkin seeds + 2 walnuts
- 2 tbsp soaked padam pisin — daily fiber + coolant
- 1 tea
- **Base ~575 kcal | ~25g protein | ~4g fiber**

**Top up to 800–1,000** with: 2 wheat bread / rice+egg / pongal + **fruit** (banana / watermelon / orange) + **banana shake or milk**. Fruit + whole grain also close the fiber gap.
> ⚠️ If 800+ in one sitting feels heavy (small stomach), split it: breakfast ~650 + a 10:30 AM snack ~300 (banana shake / nuts). Same total, gentler — ramp volume up gradually.

### Lunch (1:00–1:30 PM) — target 800–1,000
- **Chicken Buddha bowl + buttermilk + 1 tea ≈ 685 base**
- **Top up to 800–1,000:** +1.5 cup rice (+300) / +2 boiled eggs (+140) / +roasted chana or groundnuts (fiber + protein)
- **Buttermilk immediately after** (curry + coriander leaves, pinch salt) → 20–30 min → warm cumin water — gastric routine
- Office desk stash for the top-up: roasted chana, groundnuts, dates (see [[meal-log]])

### Liquid (4:00 PM — between lunch and dinner only)
- One blended drink, ~350–380 kcal, 10–14g protein — digests fast, won't compete with dinner hunger
- **Options: see the "Key Strategy — Liquid Calories" table below** (banana shake, milk+PB blend, etc.)

### Dinner (6:00–6:30 PM) — patch, ≤800, light preferred
> **No chicken or fish at dinner** (see [[profile]]). Protein = eggs, paneer, or dal.
- **Light day (~400):** dal + rice/millet + cooked veg + curd — when B+L hit ~1,000 each
- **Patch day (up to 800):** add more when B+L landed near the 800 floor, to reach ~2,400
- Dal + cooked veg = best fiber slot; prevents zero-fiber-dinner constipation (Jul 10→11)
- Small ghee on rice (1 tsp) fine; curd on the side
- **Restaurant** (no rice+dal): chapati + paneer/egg, or egg fried rice (minimal oil) + eggs
- **~400–800 kcal | 15–25g protein**

### Dinner Rules (Non-Negotiable)

| Allowed at Dinner | Never at Dinner |
|------------------|----------------|
| Plain rice / millet + dal + curd | Chicken, fish (user constraint) |
| Eggs (boiled/omelette) | Ghee roast, parotta, fried items |
| Paneer in thin curry | Heavy oil gravies, mayo |
| Cooked sabzi | Raw vegetables |
| 1 tsp ghee on rice | Deep fry anything |

---

## Key Strategy — Liquid Calories

> Liquid calories don't suppress hunger like solid food. Best tool for underweight people who struggle with appetite.

**Daily addition — drink anytime, even without hunger:**

| Option | Calories | Protein |
|--------|---------|---------|
| **Banana shake** (banana + full-fat milk + sugar) | ~300 kcal | 9g |
| **Banana shake + 1 tbsp peanut butter** (best value) | ~390 kcal | 13g |
| Full fat milk + banana blended | 350 kcal | 10g |
| Full fat milk + 2 tbsp peanut butter blended | 380 kcal | 14g |
| Full fat curd + mango/banana smoothie | 300 kcal | 8g |

One glass daily adds ~300–390 kcal without competing with meal hunger. **Breakfast note:** a banana shake is a strong lever to clear the 800 breakfast floor — easy to drink even without appetite. Keep it breakfast/daytime only (milk + banana → not near sleep, per gastric after-dinner rule).

---

## Calorie-Dense Food Additions (No Extra Hunger Required)

| Addition | Where | Extra Calories |
|---------|-------|---------------|
| Cook eggs in ghee/butter | Breakfast | +100 kcal |
| Peanut butter on bread | Breakfast | +190 kcal |
| Use full fat milk (not skimmed) | Breakfast | +50 kcal |
| Extra rice (half cup more) | Dinner | +100 kcal |
| Curd on the side every night | Dinner | +60 kcal |
| Cook with extra ghee/coconut oil | All meals | +80–100 kcal |
| Groundnuts added to lunch | Lunch | +170 kcal |

---

## Calorie-Dense Fiber Foods (Weight Gain + Fixes Constipation)

> Fiber is NOT a weight-gain driver — bulky low-cal fiber (raw salad, greens) fills you up and *sabotages* the calorie target. Get fiber ONLY from calorie-dense foods so the two goals stop fighting. All of these add gain calories AND fiber; the top ones also help your low HDL (32). See [[gastric-remedies]] fiber note.

| Food | Serving | Calories | Fiber | Why for you |
|------|---------|----------|-------|-------------|
| Avocado | 1 whole | ~240 | ~10g | Fat + fiber + raises HDL |
| Chia (soaked) | 2 tbsp | ~130 | ~10g | Omega-3 for HDL; **soak — never dry** |
| Oats in full-fat milk | 40g | ~300 | ~4g | Whole grain vs white rice; HDL |
| Moong / toor dal | 1 cup | ~200 | ~8g | Protein + fiber; **gut-gentle** (vs rajma/chana which bloat) |
| Dried figs / dates (soaked) | 3–4 | ~120 | ~4g | Fastest constipation relief + calories |
| Ragi / bajra millet | 1 dosa/roti | ~150 | ~4g | Swap for maida; fiber + calcium |
| Coconut (fresh/dried) | 30g | ~180 | ~5g | Calorie-dense; South Indian-friendly |
| Sweet potato | 1 medium | ~115 | ~4g | Low-gas dinner carb |
| Flaxseed (ground) | 1 tbsp | ~55 | ~3g | Omega-3; sprinkle on anything |
| Banana (ripe) | 1 | ~105 | ~3g | Easy daily add |

**Rule:** one fiber source *independent of the Buddha bowl* every day (fruit + dal/whole grain), or fiber collapses on non-bowl days (Jul 9 = ~11g → constipation). Ramp legumes/chia slowly and hydrate in daytime — you're gastric-prone.

---

## Protein Sources (Safe for Uric Acid)

| Food | Protein | Notes |
|------|---------|-------|
| Whole eggs | 6g/egg | 3 daily — priority |
| Chicken breast | 25g/100g | 3–4x/week |
| Fish (rohu, catla) | 20g/100g | Safe, low purine |
| Full fat curd | 10g/cup | Daily |
| Full fat milk | 8g/glass | Daily |
| Dal/lentils | 9g/cup cooked | Daily |
| Groundnuts | 7g/handful | Snack |
| Peanut butter | 8g/2 tbsp | Snack |
| Paneer | 18g/100g | 2–3x/week |

**Avoid in excess (high purine):** Mutton, prawns, organ meats, alcohol, shellfish

---

## Exercise Plan

Full plan in [[exercise-plan]] (separate file — equipment, weekly schedule, Phase 1–3 exercises, spine-safe rules, progression).

**Summary:**
- Strength 3×/week (A/B alternating) + EFX on off days — never double-session same day
- Phase 1 (Wk 1–4): floor push-ups, glute bridge, dead bug, bird dog, dumbbell row/curl/shoulder press
- Phase 2 (Wk 5–8): add goblet squat, floor chest press, Romanian deadlift
- Phase 3 (Wk 9+): confirm with Dr. Rajapandian K before progressing load

> **Post-workout nutrition:** eat within 30–45 min — milk + 2 eggs. Non-negotiable for muscle gain.

---

## Supplements

| Supplement | Dose | Reason |
|-----------|------|--------|
| Vitamin D3 (Uprise 60K) | Once/week | Level was 29.95 — insufficient |
| Omega-3 fish oil | 1g daily | HDL support + spine inflammation |
| Magnesium glycinate | 200mg at night | Sleep + muscle function + migraine |

**Whey protein:** Not needed yet — hit protein targets from whole food first. Add only if 100g protein from food proves impossible after 2 months.

---

## Timeline Reality Check (from [[meal-log]], Jul 2026)

Honest energy-balance projection based on actual logged intake — not the optimistic plan target.

**Maintenance (TDEE) ≈ 2,000–2,050 kcal/day** *(revised 2026-07-31 — training now counts)*:
- Mifflin BMR (**59.75 kg**, 184cm, 34M) ≈ **1,589 kcal**
- ⚠️ **The old "genuinely sedentary ×1.2 → ~1,900" assumption expired on Jul 25, 2026**, when [[exercise-plan]] training actually started. Current load is 3 strength sessions + 2 EFX sessions/week — that is light-activity territory, not sedentary. → ×1.3 → **~2,050/day**
- Empirical check: ate ~2,500 avg in week 1 and gained ~0.5 kg → consistent with maintenance ~1,900–2,050
- Net effect: **the real surplus at any given intake is ~100–150 kcal smaller than the old table claimed.** Gain rate is slower than previously projected, not faster.

**Projection (1 kg ≈ 7,700 kcal; Phase 1 goal +10.25 kg, 59.75 → 70):**

| Intake | Surplus over ~2,000 | Monthly gain | Note |
|--------|--------------------|--------------|------|
| 2,150 | ~150 | ~0.6 kg | Too slow now that training raised maintenance |
| **2,300** | **~300** | **~1.2 kg** | **Solid floor — this is the daily minimum** |
| **2,400** | **~400** | **~1.5 kg** | **Target rate with resistance training absorbing it** |
| 2,500+ | ~500+ | ~2 kg+ | Only justified if training volume rises further |

> **Realistic estimate: ~8–11 months** to reach the 70 kg **floor** at a consistent **~2,300–2,400/day** — then Phase 2 continues to 73–75 kg (the actual target, see [[healthspan-80]] Milestone M2). Maintenance is still low in absolute terms, so 2,300–2,400 remains the right band — but it is now a *working* surplus, not a generous one. **Do not treat 2,300 as optional.**

**The two rules that decide it:**

1. **Floor > ceiling.** One light day (~1,530) sits ~370 *below* maintenance and cancels a good day's surplus. Never drop below ~2,300 on ANY day.
2. **Resistance training, not more calories.** Sedentary + no strength work → the surplus becomes **fat, not muscle**. To gain muscle (not just belly) toward the BMI target, do the home resistance routine (see Exercise Plan) 2–3×/week. Hold intake ~2,300–2,400 — the lever now is *lifting*, not eating more.

> First 3–4 weeks may show ~1.5 kg from water/glycogen/food volume — not the true rate. Real tissue gain is the ~0.5–1 kg/month above. Weigh weekly, same time/conditions, and track the 4-week trend, not day-to-day.

---

## Monthly Milestones

> Milestones anchored to **59.75 kg (Aug 3, 2026)**, superseding the Jul 27 baseline below. Actual pace since Jul 6 is running ~1.9 kg/month, not the ~1 kg/month these milestones assume — see [[meal-log]] checkpoint (2026-08-03) for the trim recommendation. Milestone dates below are not yet re-projected for the faster pace.

### Phase 1 — 59.75 kg → 70 kg (BMI 17.6 → 20.7)

| Month | Approx Date | Target Weight | Focus |
|-------|-------------|--------------|-------|
| 1 | Aug 2026 | ~60 kg | Consistent meal floor — no light days |
| 2 | Sep 2026 | ~61 kg | Start resistance training (see [[exercise-plan]]) |
| 3 | Oct 2026 | ~62 kg | Retest uric acid + lipid profile (HDL baseline) |
| 5 | Dec 2026 | ~64 kg | HDL starting to respond to exercise + diet |
| 8 | Mar 2027 | ~67 kg | Strength gains visible; reassess supplements |
| 11 | Jun 2027 | **~70 kg** | **Phase 1 complete — reassess body composition** |

### Phase 2 — 70 kg → 73–75 kg (BMI 20.7 → 21.5–22.2)

> Phase 2 is the better long-term target for 184 cm. At 70 kg you're still lean — 73–75 kg adds the muscle mass that improves HDL, joint support, and spine stability meaningfully. Gain rate slows as BMI normalises (~0.5–0.8 kg/month).

| Month from Phase 1 | Approx Date | Target Weight | Focus |
|--------------------|-------------|--------------|-------|
| +2 | Aug 2027 | ~71.5 kg | Assess: does 70 kg feel lean? Continue or consolidate |
| +5 | Nov 2027 | ~73 kg | Retest full lipid panel — HDL should be climbing |
| +8 | Feb 2028 | **~75 kg** | **Phase 2 complete — full target BMI 22.2** |

---

## Lab Tests to Track Progress

**At 3-month mark:**
- CBC
- Uric acid (ensure protein increase hasn't spiked it)
- Lipid profile (track HDL improvement from exercise)
- Vitamin D

---

## Biggest Risk to This Plan

**The light day** — the single failure mode. Full explanation in the "Floor > ceiling" rule under [Timeline Reality Check](#timeline-reality-check-from-meal-log-jul-2026) (one light day erases two good days; never below ~2,300). The fixes below are how you prevent it:

**Fix:** Set phone alarms for:
- 4:00 PM — liquid meal (milk + banana blend)
- 6:00 PM — start dinner before cutoff

**Snacks — revised (Jul 2026):** The original plan said "no snacks between meals" to protect digestion. The meal-log overrules that: **undereating is the bigger, proven risk than digestion load.** On light days, three meals do not reach 2,400 kcal. So:
- **Allowed:** low-volume, calorie-dense snacks that don't burden the gut — a handful of nuts, soaked dried figs/dates, a chikki/laddu, or a liquid blend. These add ~400–500 kcal + fiber without a heavy meal's digestion cost.
- **Still avoid:** heavy/fried/large-volume snacks close to a meal (those do trigger the gas/headache pattern).
- The point of the original rule (protect gastric digestion) is preserved by choosing *what* to snack on, not by skipping snacks and undereating.

---

## Doctors to Consult

| Doctor | Question |
|--------|---------|
| Dr. Rajapandian K (Ortho) | "Uric acid is now 5.3 — can I eat protein freely? Which strength exercises are safe for my spine?" |
| General Physician | Confirm no malabsorption issue if weight doesn't increase after 2–3 months of eating more |

---

## Related
- [[profile]] — canonical health profile
- [[meal-log]] — daily calorie / protein / fiber tracking (source for the reconciliation above)
- [[gastric-remedies]] — fiber gap, digestion, after-dinner rule
- [[lab-reports-summary]] — lab history
