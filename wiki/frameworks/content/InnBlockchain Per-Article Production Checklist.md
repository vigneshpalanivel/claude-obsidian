...
| # | Step | Doc to consult |
|---|---|---|
| 1 | **(CW)** Confirm this week's slot — article ID, tier (1/2/3/0), score, track tag (`[CN]` / `[FT]` / `[Both]`), gate dependencies | [[Editorial Calendar]] § (phase-specific) Close Schedule |
| 2 | **(CW · escalate to Dev if 404)** Verify destination LP is live — if 404, escalate as P0 before drafting (Tier 0 rule: no repurposing on LP-gated articles) | [[Landing Pages Plan]] § matching solution sub-page |
| 3 | **(CW)** Confirm article positioning — pillar / cluster · primary persona · sales motion · `[Audit]` / `[Dev]` / `[White-Label]` motion lock | [[Content Strategy]] § Pillar Map + § Sales Motion Tagging |
| 4 | **(CW)** Confirm persona language for this track | [[ICP - Crypto Native]] OR [[ICP - FinTech]] (whichever the track tag points to) |
...
| 6 | **(CW)** Pull per-article AEO prompt block + apply 13 AEO Drafting Principles | [[Content AEO GEO Prompts]] § Week-by-Week Article Prompts + § AEO Drafting Principles |
| 7 | **(CW)** Confirm derivative chain scope by tier — Tier 1 full chain · Tier 2 scaled-back · Tier 3 amplify-only · Tier 0 none. Plan derivative copy structure while outlining. | [[Content Repurposing Plan]] § Repurposing Tiers |

## Phase 2 — Write (Mon PM for Tue slot · Wed for Thu slot · ~3 hr)

| # | Step | Doc to consult |
|---|---|---|
| 8 | **(CW)** Outline per AEO prompt block (H2/H3 = prompts verbatim per Principle 2) | [[Content AEO GEO Prompts]] § AEO Drafting Principles |
...
| 11 | **(CW)** Apply voice / tone / formatting / citation rules per Style Guide | [[Content Style Guide]] |
| 12 | **(CW)** If `[Both]` article: confirm split CTA footer routes correctly to per-persona LP destinations · If regulatory-boundary article: add external-link routing footer per phase rules | [[Landing Pages Plan]] + Playbook WS1 routing rules |
...
> Each gate is a checkbox. Skip a gate = wasted production (wrong tier · wrong format · wrong audience · wrong CTA). Was previously WS15 § Per-published-article repurposing checklist in the playbook; moved here as the step 19 expansion in rev 29.
...
- [ ] **(CW)** **Step 3: Track gate** — `[CN]` → Track A asset family · `[FT]` → Track B asset family · `[Both]` → parallel chains per [[Content Repurposing Plan]] § `[Both]` Articles
...
| 22 | **(MPs produce visuals + post · Designer for high-stakes)** Derivative ASSETS ship per LinkedIn output cap cadence — copy was structured in step 19, visuals produced now per Tier 1 full chain / Tier 2 lite / Tier 3 amplify-only | [[Cross-Channel Cannibalisation Matrix]] + Playbook WS15 |
| 23 | **(MPs enforce in scheduling)** Verify LinkedIn output cap — ≥48 hr spacing between any two posts about same article · ≤7 posts/wk steady state · cross-pillar Tier 1 distributes across 3 weeks (not launch week) | [[Cross-Channel Cannibalisation Matrix]] vectors 3, 4, 6 |
