...
parent_extraction_source: "[[Phase 1 Execution Playbook]] rev 26 § WS1 § Vignesh Pre-Launch LP Sign-Off Checklist"
scope: cross-phase
applies_to:
  - phase-1
  - phase-2
...
>
> Extracted from [[Phase 1 Execution Playbook]] rev 26 → cross-phase framework (rev 27). Phase 1 fires this checklist for Gate 1 + Gate 2 + eIDAS gated PDF; Phase 2 will fire for new destination LPs.
...
- [ ] **(Vignesh + Dev verification)** **CTA copy + destination targeting** — primary CTA + secondary CTA both point to the correct destination per [[Landing Pages Plan]]. **No CTA points to a non-existent LP** (broken-CTA-to-future-phase-LP = Carol-killing trust drop).
...
- [ ] **(Vignesh)** **Cross-doc consistency check** — no claims contradict [[LinkedIn Profile Vignesh Content]] Pre-Publish Hard Gate. No claims contradict [[Content Strategy]] § sales motion tagging for the destination LP.
...
- [ ] **(MPs run probe · Vignesh reviews results)** AEO citation probe per [[Landing Pages AEO GEO Prompts]] § Monthly AI Search Citation Probe — does Perplexity / ChatGPT now cite the LP for vendor-evaluation queries within 48 hr? (Indexing lag is normal; 48-hr check confirms LP is reachable)
