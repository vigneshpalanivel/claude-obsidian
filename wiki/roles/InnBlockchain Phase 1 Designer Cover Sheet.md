...
> **What this is:** Your slice of [[InnBlockchain - Phase 1 Execution Playbook]] for daily/weekly use. ~26 task touchpoints across 4 workstreams. **Part-time engagement** — concentrated W2–W3 brand system + Sample Audit Report sprint; then ~5–10 hr/mo on high-stakes pieces.

---

## 🎯 What you own (top-level)

**High-stakes pieces only** — credibility hinges on human design polish, not AI-template output. Sample Audit Report PDF (single highest-leverage Phase 1 production item) · Vendor Risk Pack PDF · brand template system setup (W2–W3 one-time) · compliance brief PDF layouts (MiFID II/III · DORA · eIDAS 2.0) · complex regulatory infographics · W14 Internal MVP mockup fallback (conditional trigger). Day-to-day AI visual production (carousels · quote graphics · hero images) lives with MPs using YOUR templates.
...
| **WS15 — Content Repurposing** | High-stakes pieces only — complex regulatory infographics · Sample Audit Report annual refresh (anchor-tied to first-delivered engagement) · brand template enforcement (catch MPs drift) | [[InnBlockchain - Phase 1 Execution Playbook#WS15 — Content Repurposing]] |
...
| **W1–W3 (~5 days)** | **Brand system foundation deliverables**: typography system · color palette · logo usage rules · Canva carousel + quote graphic templates · Beautiful.ai lead magnet template · Descript caption styles · LinkedIn profile banner · X header template · Midjourney hero image style guide · MPs handoff verification |
| **W2–W3 (~5 days)** | **Sample Audit Report PDF (HIGHEST-LEVERAGE)** + Risk Checklist PDF + Vendor Risk Pack PDF assembly |
| **W7 (~3–4 days)** | DORA brief PDF layout (for P5-DORA-X article) |
...
| **Quarterly** | Brand consistency review with MPs (drift correction · new template requests routed to you) |
| **Ongoing (~5–10 hr/mo)** | High-stakes pieces (complex regulatory infographics · conference deck slides for high-value events · Sample Audit Report annual refresh when triggered) |

## 📚 Read first (in this order)

1. [[InnBlockchain - Phase 1 Execution Playbook#Brand system foundation (Designer W2–W3 sprint — pre-Featured-asset gate)]] — **your W2–W3 sprint deliverables checklist**
...
- **Sample Audit Report PDF is the single highest-leverage Phase 1 production item** — never ship without Vignesh compliance sign-off. Gates $10M+ Sasha + Track B Tom + Carol pre-call trust.
- **Synthetic Sample Audit MUST carry front-matter disclosure** — exact wording: "Sample reference report — methodology and findings drawn from publicly available exploit research; no client engagement." Synthetic-without-disclosure = reputational + regulatory risk.
- **Never publish Sample Audit Report or Vendor Risk Pack designed by anyone other than you** (per Toolkit hard rules) — MPs cannot AI-template these.
- **Brand template is the source of truth — no ad-lib visual elements.** When MPs request a new template variant, you build it; they don't create ad hoc in Canva.
- **Never use AI avatar video** (Synthesia / HeyGen) for CEO-positioned content — too obvious a synthetic tell.
...
- Brand standards drift detected in MPs work → review + correct + add to quarterly brand audit
- Sample Audit Report content question → **Vignesh** (he authors content; you design layout)
- Regulatory framing uncertainty in infographic or compliance brief layout → **Vignesh** (same day)
- W14 client deployment status (MVP fallback trigger decision) → **Vignesh decides** at W14 review
- Capacity overload → flag to **Vignesh + MPs** — defer non-high-stakes pieces; never compromise Sample Audit / Vendor Risk Pack quality

## ❌ What you DON'T own (redirect if asked)

- Day-to-day AI visual production (carousels · quote graphics · single-image visuals) → **MPs** (using your templates)
- Brand template maintenance ongoing → **MPs** (you build; they maintain via templates)
- LP build / integration → **Dev** (you provide hero images; he integrates)
- Article content writing → **CW**
- Outreach / DMs / discovery calls → **SP**
- Final regulatory sign-off on PDFs → **Vignesh** (you design + lay out; he signs off on compliance)
