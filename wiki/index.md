...
- [[RWA-STO-EU-Compliance-Criteria]] — visual criteria companion to the RWA/STO landscape: 19 Mermaid decision trees (token classification, MiCA sub-classes, role stack, venue type, client classification, suitability tiers, prospectus formats, MAR/PDMR, UCITS→AIF→ELTIF, AIFMD levels, LOF, DLT Pilot caps, DORA, AMLR CDD, TFR, eIDAS) with article refs + thresholds in nodes; sourced from landscape + all 15 checklists (status: current)
